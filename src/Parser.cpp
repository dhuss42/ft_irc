...
bool Client::handleCap(Message message)
{
	...
	{
		sendMsg("irc_custom", "List of available capabilities: farting");
	...
	{
		sendMsg("irc_custom", "join works, accepted"); //improve this reply maybe
		return (true);
	}
...
bool Client::handleUser(Message message)
{
	...
	sendMsg("irc_custom", "Welcome to our super nice IRC-Server! <3");
	return (true);
}
